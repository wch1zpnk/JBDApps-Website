...
- Smartphone visitors are detected in `games/game.js`: iPhone/iPod user agents and Android user agents containing `Mobile` receive the phone layout, while Android tablets and desktop browsers remain on the standard layout. Portrait orientation shows a dedicated animated `Rotate Your Smartphone` screen, makes the hidden game inert, clears held controls, and freezes the update loop so gameplay cannot advance unseen. Rotating to landscape automatically removes the gate and resumes the same game state. The landscape layout fills the safe viewport, centers the 4:3 playfield, hides the keyboard instruction strip, and places the four touch controls in the side space. `?smartphone-preview=1` is the current desktop testing override; the older `?iphone-preview=1` remains supported for compatibility.
- The smartphone game surface suppresses browser text selection, touch callouts, tap highlighting, drag selection, context menus, and synthesized double-click defaults. Touch controls use `touch-action: none`; hold-button pointer-up/cancel/leave handlers also cancel browser defaults. This prevents iOS from drawing blue selection bars or selecting nearby game text when Fire is tapped rapidly, while semantic buttons and keyboard focus behavior remain intact.
...
- The Apps page uses matching top-right Mac App Store buttons for Everything Clipboard and Voice Command Atlas. The Voice Command Atlas listing card uses the same purple-tinted background and border treatment as its homepage showcase card.
- Every `apps.apple.com` link in public HTML opens in a new tab with `target="_blank"` and `rel="noopener noreferrer"`, keeping the JBDApps page open. This is a site-wide rule for future Mac App Store links.
...
- 2026-07-22 Galactic Swarm smartphone touch-selection fix completed:
  ...
  - Local and live production stress tests each issued three rapid Fire double-click sequences and confirmed empty selected text, zero selection ranges, `user-select: none`, `touch-action: none`, no overflow, and zero console errors.
  - `git diff --check`, both game JavaScript syntax checks, shared-site JavaScript syntax check, sitemap XML validation, exact local/public comparisons, and HTTPS SHA-256 readback passed. GitHub Pages run `29941879151` succeeded.
  - Live verification confirmed versioned `20260722-touch-selection-fix` assets and no selection artifact after repeated Fire interaction.

- 2026-07-22 Galactic Swarm smartphone expansion completed:
  - Commit `03fbdb7 Extend landscape mode to smartphones` generalized the mobile gate from iPhone/iPod to iPhone, iPod, and Android phones and changed all visible copy to `Smartphone Game`, `Rotate Your Smartphone`, and `Turn your smartphone sideways to play Galactic Swarm in landscape.`
  - A deterministic user-agent check passed for iPhone Safari and Android Chrome phone examples, excluded an Android tablet and Mac Safari, and preserved the `?iphone-preview=1` testing override alongside the new `?smartphone-preview=1` override.
  - Local portrait and landscape browser verification confirmed the generic smartphone classes, instruction, inert gate, automatic transition, touch controls, hidden desktop strip, no overflow, and zero console errors.
  - `git diff --check`, both game JavaScript syntax checks, exact local/public source comparisons, and HTTPS SHA-256 readback passed. GitHub Pages run `29941281588` succeeded.
  - Live portrait verification confirmed the generic wording, versioned `20260722-smartphone-landscape` assets, inert game shell, and zero console errors.
...
- 2026-07-22 Galactic Swarm compact-HUD and automatic-upgrade publication completed:
  ...
  - Clearing a wave now chooses one entry randomly from the existing seven run upgrades, applies it immediately, starts the next wave, and briefly identifies the applied upgrade in the wave banner.
  - `/Users/briscoe/Documents/Galaga/web/{index.html,style.css,game.js}` and the website `games/` copies are byte-for-byte identical. The HTML cache-busts both assets with `20260722-compact-auto-upgrade`.
  - `git diff --check`, both game JavaScript syntax checks, the shared-site JavaScript syntax check, sitemap XML validation, stale-choice-code scan, and no-global-shake scan passed before publication.
  - GitHub Pages run `29936756563` completed successfully. HTTPS returned `HTTP/2 200`, and SHA-256 readback proved that all three deployed files exactly match the local originals.
  - Live in-app-browser verification measured the deployed HUD at 34 pixels tall, found no upgrade panel or horizontal overflow, confirmed the two versioned asset URLs, watched the demo advance automatically from wave 1 to wave 3, and logged zero console errors.
...
- 2026-07-18 Apps-page color/button matching and site-wide App Store new-tab update completed:
  ...
  - All nine `apps.apple.com` links across `/`, `/apps/`, `/apps/everything-clipboard/`, and `/apps/voice-command-atlas/` include `target="_blank"` and `rel="noopener noreferrer"`.
  - A real local browser click on the Everything Clipboard top-right button opened a separate App Store tab while the original JBDApps Apps page remained at `/apps/`; the temporary test tab was then closed.
  - `git diff --check`, `node --check assets/js/main.js`, `xmllint --noout sitemap.xml`, exact Apple-link counts, missing-target scans, missing-rel scans, and `macappstore://` scans passed before publication.
  - GitHub Pages run `29665396642` completed successfully for functional commit `aae7401`; the only annotation was the existing non-blocking Node.js 20 deprecation note.
  - Live cache-busted in-app-browser verification confirmed the new stylesheet token, matching buttons, purple Atlas card, three correctly attributed App Store links on `/apps/`, zero horizontal overflow, zero failed images, and zero console warnings/errors.
  - Live cache-busted HTML readback confirmed all nine App Store links across the four affected pages retained both required new-tab attributes.
...
- Keep the public Galactic Swarm gameplay code synchronized with the original local source when publishing approved changes. Preserve smartphone-wide selection/callout suppression on the touch controls, the smartphone portrait rotation gate and Android-phone detection, landscape safe-area/touch layout, compact single-strip HUD, automatic random wave upgrades without a choice screen, all-wave ship-collision damage, the stronger localized player explosion, and the stationary whole-canvas behavior.
